...
def parse_rados_bench(group):
    ...
    for results_file in group:
        ...
        dir_name = results_file.split(os.sep)[-2]
        iotype = dir_name.split('.')[0]
        parsed_io[iotype]
        curr_MB = []
        avg_lat = -1.0
        min_lat = -1.0
...
def sum_results(results):
    ...
    for key in results.iterkeys():
        ...
        for data in results[key]:
            ...
            avg_lat += data[3]
        print(key)
        # 6 is a "magic" number of data nodes in the cluster, one day we will read this in from the yml file
        bandwidth = calc_per_node_bw(bw,6)
        lat = (min_lat/len(results[key]), max_lat/len(results[key]), avg_lat/len(results[key]))
        print("%s-%3.5f %3.5f %3.5f" % (key,lat[0],lat[1],lat[2]))
        print("%s-%s" % (key,bandwidth))
        #print(calc_per_node_bw(bw,6)," %3.5f %3.5f %3.5f"% (min_lat/len(results[key]), max_lat/len(results[key]), avg_lat/len(results[key])))
